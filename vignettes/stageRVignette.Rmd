...
---
  ...
  This vignette describes the stageR package that has been developed for stage-wise analysis of high throughput gene expression data in R. A stage-wise analysis was shown to be beneficial in terms of biological interpretation and statistical performance when multiple hypotheses per gene are of interest [@VandenBerge2017].
  The stage-wise analysis has been adopted from [@Heller2009] and consists of a screening stage and a confirmation stage. In the screening stage, genes are screened by calculating p-values that aggregate evidence across the different hypotheses of interest for the gene. The screening p-values are then adjusted for FDR control upon which significance of the screening hypothesis is assessed.
  ...
  `stageR` provides an automated way to perform stage-wise testing, given p-values for the screening and confirmation stages. Alternatively, only the p-values for the confirmation stage can be provided, and we have implemented a number of procedures to calculate the aggregate p-value. A number of FWER control procedures that take into account the logical relations among the hypotheses are implemented. Since the logical relations may be specific to the experiment, the user can also specify an adjustment deemed appropriate.
  
  The vignette analyses two datasets. The Hammer dataset [@Hammer2010] is a differential gene expression analysis for an experiment with a complex design. This type of analyses are supported by the `stageR` class. The Ren dataset [@Ren2012] analyzes differential transcript usage (DTU) in tumoral versus normal tissue in Chinese patients. Transcript-level analyses are supported by the `stageRTx` class. 
...
The development version of the package is hosted on GitHub and can be installed with the `devtools` library using `devtools::install_github("statOmics/stageR")`. A couple of stable releases can be found in the 'Releases' tab on GitHub.
...
The Hammer experiment investigated the effect of an experimentally induced spinal nerve ligation (SNL) versus control samples in rats at two weeks and two months after treatment. For every time $\times$ treatment combination, 2 biological replicates were used. The hypotheses of interest are
...
The data is then normalized with TMM normalization [@Robinson2010] to correct for differences in sequencing depth and RNA population between the samples.

```{r}
cpmOffset <- 2
keep <- rowSums(cpm(assayData(eset)$exprs)>cpmOffset)>=2 #2cpm in 2 samples
dge <- DGEList(assayData(eset)$exprs[keep,])
...
We will first analyze the data with limma-voom [@Law2014] in a standard way: the three contrasts are assessed separately on an FDR level of $5\%$. 
...
```{r}
padj <- getAdjustedPValues(stageRObj, onlySignificantGenes=FALSE, order=FALSE)
head(padj)
```

and may either return all p-values or only those from the significant genes, as specified by the `onlySignificantGenes` argument which can then be ordered or not as specified by the `order` argument. The output returned by `getAdjustedPValues` is a nested data frame where every row represents the data for one gene. In the output above, only the aggregated p-value (`genePval`) and the FDR-adjusted aggregated p-value (`genePadj`) are immediately visible. The results for the three different contrasts are hidden in the nested `data` column, and can be retrieved by subsetting the nested data frame on that column, as we do below.

```{r}
head(padj$data, n=3)
```

The first element of this list corresponds to the first row (hence gene) of the `padj` data.frame we have previously defined. Every element is a data frame structure where now every row corresponds to one hypothesis for the gene. Note, that for the first gene, three different p-values are returned for every hypothesis: `pvalue`, `padj` and `padj_SW`. `pvalue` represents the raw confirmation stage p-values for that gene. `padj` represents the FWER adjusted p-values, and `padj_SW` are the FWER adjusted p-values that have also been corrected to the BH-adjusted significance level of the screening stage. **Hence, `padj_SW` may directly be compared to the requested OFDR level (here, $5\%$).** Note that for the second gene, only raw p-values are returned. This is because this gene has not passed the screening stage, so we are unable to (and it would be irrelevant to) calculate stage-wise adjusted p-values.
...
Using stage-wise testing, we now find `r colSums(res)[4]` significant genes for the interaction effect!
...
More conservative results are obtained when adopting Holm's method. But, this method is generally applicable to all experimental layouts.

```{r}
adjustedPSW <- stageWiseAdjustment(object=stageRObj, method="holm", alpha=0.05)
res <- getResults(adjustedPSW)
colSums(res)
```
...
geneForEachTx <- tx2gene[match(rownames(esetProstate),tx2gene[,1]),2]
dxd <- DEXSeqDataSet(countData = assayData(esetProstate)$exprs,
...
### Gene-level p-values precalculated
...
We can then explore the results using a range of accessor functions. The significant genes can be returned with the `getSignificantGenes` function. The output of this function is very similar to the one from `getAdjustedPValues` we have described above.

```{r}
head(getSignificantGenes(stageRObj))
nrow(getSignificantGenes(stageRObj))
```

Similar, the significant transcripts can be returned with `getSignificantTx`. The output is now no longer collapsed by gene, and all significant transcripts can be found in the data frame. Note, that, again, three different p-values are returned for every hypothesis: `pvalue`, `padj` and `padj_SW`. `pvalue` represents the raw confirmation stage p-value for that transcript. `padj` represents the FWER adjusted p-values, and `padj_SW` are the FWER adjusted p-values that have also been corrected to the BH-adjusted significance level of the screening stage. **Hence, `padj_SW` may directly be compared to the requested OFDR level (here, $5\%$).**

```{r}
head(getSignificantTx(stageRObj))
nrow(getSignificantTx(stageRObj))
```


The stage-wise adjusted p-values are returned using the `getAdjustedPValues` function. The screening (gene) hypothesis p-values were adjusted according to the BH FDR criterion, and the confirmation (transcript) hypothesis p-values were adjusted to control for the full stage-wise analysis, by adopting the correction method specified in `stageWiseAdjustment`. `getAdjustedPValues` returns a data frame where the different rows correspond to different genes. Transcript-level adjusted p-values can be found in the `data` column of the output. Note, that the stage-wise adjusted p-values are only valid for the provided significance level and must not be compared to a different significance level. If this would be of interest, the entire stage-wise testing adjustment should be re-run with the other significance level provided in `alpha`.

```{r}
padj <- getAdjustedPValues(stageRObj, order=TRUE, onlySignificantGenes=FALSE)
head(padj)
head(padj$data,2) #transcript-level results for the first two genes in padj.
```

### Aggregate transcript-level p-values with stageR

DEXSeq uses Sidak's method to aggregate p-values across transcripts within a gene. Recently, a number of other procedures have been adopted for p-value aggregation [@Yi2018], which are typically more powerful than the DEXSeq procedure. stageR now allows you to set up a `stageR` object by only providing transcript-level p-values and specifying a method of choice on how to aggregate the transcript-level p-values to the gene level. The method for aggregation can be specified using the `aggMethod` argument in the `stageWiseAdjustment` function. Currently, three methods are available: Sidak, Fisher and Lancaster's method, as implemented in the aggregation R package. For more details on these aggregation methods, we refer to [@Yi2018]. Below, we use Fisher's method for p-value aggregation, which results in a more than threefold increase in the number of significant genes and transcripts.

```{r}
stageRObj <- stageRTx(pConfirmation=pConfirmation, tx2gene=tx2gene)
stageRObj <- stageWiseAdjustment(object=stageRObj, method="dtu", alpha=0.05, aggMethod="fisher")
nrow(getSignificantGenes(stageRObj))
nrow(getSignificantTx(stageRObj))
```

Lancaster's method does a weighted p-value aggregation. Yi et al. (2018) foudn that it is powerful when the weights correspond to the base mean of the transcripts according, which is more or less their mean across samples. Lancaster's method can be used in stageR if the weights are provided. Note that the weights must be in the same order as the rows in `pConfirmation`.

```{r}
# make sure baseMean is in same order as pConfirmation.
baseMean <- dxr$exonBaseMean
names(baseMean) <- substr(x=names(baseMean),17,31)
baseMean <- log1p(baseMean[rownames(pConfirmation)])
stageRObj <- stageRTx(pConfirmation=pConfirmation, tx2gene=tx2gene)
stageRObj <- stageWiseAdjustment(object=stageRObj, method="dtu", alpha=0.05, aggMethod="lancaster", weights=baseMean)
nrow(getSignificantGenes(stageRObj))
nrow(getSignificantTx(stageRObj))
```

As a note of caution, note that the weights are crucial. For example, we find fewer genes and transcripts if we do not log the baseMean, as shown below.

```{r}
# make sure baseMean is in same order as pConfirmation.
baseMean <- dxr$exonBaseMean
names(baseMean) <- substr(x=names(baseMean),17,31)
baseMean <- baseMean[rownames(pConfirmation)]
stageRObj <- stageRTx(pConfirmation=pConfirmation, tx2gene=tx2gene)
stageRObj <- stageWiseAdjustment(object=stageRObj, method="dtu", alpha=0.05, aggMethod="lancaster", weights=baseMean)
nrow(getSignificantGenes(stageRObj))
nrow(getSignificantTx(stageRObj))
